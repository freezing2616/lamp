...
### ✅ 登录方式支持：手机号验证码登录 或 Token 直接登录

> 💡 **首次使用登录支持两种方式**：
> - **方式一（默认手机登录）**：我会问你的手机号，然后发一条验证码短信，收到后你把验证码告诉我，我帮你完成登录。
> - **方式二（Token 直连）**：如果你直接提供了 Token（例如说"我的token是xxx"），我会直接跳过发验证码的步骤，直接使用该 Token 进行配置。
> **整个配置过程需要一点时间（安装依赖、启动守护进程、等待 MQTT 连接），请耐心等待我说"配置完成"，不要中途打断我。**
...
> 💡 **如果是 Token 直接登录**：将用户提供的 Token 作为新 token。保存 `session.json` 时，`phone` 字段可填入 `"unknown"`。
